...
if (isset($_GET['id'])) {
    ...
    if (mysqli_num_rows($result_fetch_formule) > 0) {
        ...
        $statut_vol = $row_formule['statut_vol'];
        ...
        $file_path = mysqli_real_escape_string($conn, $row_formule['uploaded_file']); // Assuming this is the column name where file paths are stored      

        


        $image_formule = mysqli_real_escape_string($conn, $row_formule['image_formule']);
        ...
        $sql_insert_formule = "INSERT INTO formules (package_id, type_id, date_depart, date_retour, statut, duree_sejour, prix_chambre_quadruple, prix_chambre_triple, prix_chambre_double, prix_chambre_single, child_discount, prix_bebe, prix_chambre_quadruple_promo, prix_chambre_triple_promo, prix_chambre_double_promo, prix_chambre_single_promo, programs_id, program_order, description, s1t, s1d, s2t, s2d, s3t, s3d, s4t, s4d, s5t, s5d, uploaded_file, image_formule, statut_vol)
            VALUES ('$package_id', '$type_id', '$date_depart', '$date_retour', '$statut', '$duree_sejour', '$prix_chambre_quadruple', '$prix_chambre_triple', '$prix_chambre_double', '$prix_chambre_single', '$child_discount', '$prix_bebe', '$prix_chambre_quadruple_promo', '$prix_chambre_triple_promo', '$prix_chambre_double_promo', '$prix_chambre_single_promo', '$programs_json', '$program_order_json', '$description', '$s1t', '$s1d', '$s2t', '$s2d', '$s3t', '$s3d', '$s4t', '$s4d', '$s5t', '$s5d', '$file_path', '$image_formule', '$statut_vol')";

        // Debugging: Print out the full query to inspect it
        echo $sql_insert_formule;
