...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if (mysqli_query($conn, $sql_duplicate_package)) {
        ...
        if (mysqli_query($conn, $sql_duplicate_formules)) {
            ...
            $sql_get_original_formules = "SELECT id FROM formules WHERE package_id = $package_id";
            ...
            if (mysqli_num_rows($result_original_formules) > 0) {
                while ($row_original_formule = mysqli_fetch_assoc($result_original_formules)) {
                    ...
                    $sql_get_new_formule = "SELECT id FROM formules WHERE package_id = $new_package_id AND id NOT IN (" . implode(",", $new_formule_ids) . ") LIMIT 1";
            ...
            // Step 4: Duplicate vols for the new formules
            foreach ($original_to_new_formule_ids as $original_formule_id => $new_formule_id) {
                ...
                // Step 5: Duplicate hebergements for the new formules
