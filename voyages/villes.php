...
// Check if category_parent_id is provided
$categoryParentId = isset($_GET['category_parent_id']) ? $_GET['category_parent_id'] : null;

// Fetch packages based on the selected category
if ($categoryParentId) {
  $packagesQuery = "SELECT * FROM omra_packages WHERE category_parent_id = $categoryParentId";
} else {
  // Default to all packages if no category is selected
...
if ($packagesResult->num_rows > 0) {
  while ($package = $packagesResult->fetch_assoc()) {
    ...
    // Fetch formules for the current package
$formulesQuery = "
SELECT f.*, t.nom as type_nom, h.etoiles, ca.logo as compagnie_logo
FROM formules f
LEFT JOIN type_formule_omra t ON f.type_id = t.id
LEFT JOIN hebergements e ON f.id = e.formule_id
LEFT JOIN hotels h ON e.hotel_id = h.id
LEFT JOIN vols v ON f.id = v.formule_id
...
"><b>       <?php echo $formule['type_nom']; ?></b></p>
                  ...
                  <p style="color: #d9c294;"><i class="fa-solid fa-sun">&nbsp;</i> <span style="color: #000;">
                      ...
                      Jour(s) </span></p>                      
                      <img src="../<?php echo $formule['compagnie_logo']; ?>" alt="Logo de la compagnie aérienne" style="width: 100px; float: right;">
                  <div style="margin-bottom:5px;">
                    ...
                    <div style="float:right; width:50%; text-align:right; margin-top: -40px;">
