...
// Get the category_parent_id from the URL
$category_parent_id = isset($_GET['category_parent_id']) ? intval($_GET['category_parent_id']) : 0;

if ($category_parent_id == 0) {
    echo "Invalid Category Parent ID.";
    exit;
}

$sql_category = "SELECT nom FROM category_parent WHERE id = $category_parent_id";
$result_category = mysqli_query($conn, $sql_category);

if ($result_category && mysqli_num_rows($result_category) > 0) {
    $row_category = mysqli_fetch_assoc($result_category);
    $category_name = $row_category['nom'];
} else {
...
<div class="container">
    ...
    <h2 class="text-center">Liste des villes de la catégorie <?php echo $category_name; ?></h2>
    ...
    <table>
        ...
        <tbody>
            <?php
            // SQL query to select all Omra packages for the selected category_parent
            $sql_packages = "SELECT omra_packages.*, category_parent.nom AS category_name
                            ...
                            WHERE omra_packages.category_parent_id = $category_parent_id";
            ...
            if (mysqli_num_rows($result_packages) > 0) {
                ...
                echo "<tr><td colspan='5'>Aucun package Omra disponible pour cette catégorie parent.</td></tr>";
            }
