...
if (isset($_GET['id'])) {
    $category_parent_id = $_GET['id'];

    // Récupérer les données du package à partir de la base de données
    $sql = "SELECT * FROM category_parent WHERE id = $category_parent_id";
...
if (isset($_POST['update_category_parent'])) {
    ...
    $sql = "UPDATE category_parent SET 
            nom = '$nom', 
...
<head>
    ...
    <title>Modifier une Catégorie Parent</title>
...
<body>
    ...
    <div class="container">
        <h2>Modifier une Catégorie Parent</h2>
            <form action="edit_categorie_parent.php?id=<?php echo $category_parent_id; ?>" method="POST" enctype="multipart/form-data">
                ...
                <button type="submit" name="update_category_parent">Modifier Catégorie Parent</button>
            </form>
    </div>
